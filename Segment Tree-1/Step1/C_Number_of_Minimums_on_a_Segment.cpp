// Gurmehar Singh 

#include<bits/stdc++.h>
#include<algorithm>
// #include<ext/pb_ds/assoc_container.hpp>
// #include<ext/pb_ds/tree_policy.hpp>
#define yes cout<<"Yes"<<"\n"
#define no cout<<"No"<<"\n"
#define nl cout<<"\n"
#define all(a) a.begin(),a.end()
#define ivec(vec) for(int i=0;i<vec.size();i++){cin>>vec[i];}
#define ovec(vec) for(int i=0;i<vec.size();i++){cout<<vec[i]<<" ";}
#define pb push_back
#define int long long
using ll=long long;
using ull=unsigned long long;
const int mod=1e9+7;
using namespace std;
// using namespace __gnu_pbds;
// typedef tree<int,null_type,less<int>,rb_tree_tag,tree_order_statistics_node_update> pbds;  
// // (functions find_by_order, order_of_key)

class SegmentTree
{
    public:
    vector<pair<int,int>> seg;
    
    SegmentTree(int n)
    {
        seg.resize(4 * n + 1);

        
    }
    void build(int index, int low, int high, vector<int>& arr)
    {
        if (low == high)
        {
            seg[index] = {arr[low],1};
            return;
        }
        int mid = (low + high) / 2;
        build(2 * index + 1, low, mid, arr);
        build(2 * index + 2, mid + 1, high, arr);
        if(seg[2*index+1].first < seg[2*index+2].first){
            seg[index].first = seg[2*index+1].first;
            seg[index].second = seg[2*index+1].second;
        }
        else if(seg[2*index+1].first > seg[2*index+2].first){
            seg[index].first = seg[2*index+2].first;
            seg[index].second = seg[2*index+2].second;

        }
        else{
            seg[index].first = seg[2*index+1].first;
            seg[index].second = seg[2*index+1].second + seg[2*index+2].second;
        }
    }

    pair<int,int> query(int index, int low, int high, int l, int r)
    {
        if (r < low || high < l)
            return {INT_MAX,1};

        if (low >= l && high <= r)
            return seg[index];

        int mid = (low + high) / 2;

        pair<int,int> left = query(2 * index + 1, low, mid, l, r);
        pair<int,int> right = query(2 * index + 2, mid + 1, high, l, r);
        if(left.first < right.first) return {left.first,left.second};
        else if(left.first > right.first) return {right.first,right.second};
        return {left.first,left.second + right.second};
    }

    void update(int ind, int low, int high, int i, int val)
    {
        if (low == high)
        {
            seg[ind].first = val;
            seg[ind].second=1;
            return;
        }
        int mid = (low + high) >> 1;
        if (i <= mid)
            update(2 * ind + 1, low, mid, i, val );
        else
            update(2 * ind + 2, mid + 1, high, i, val);
        if(seg[2*ind+1].first < seg[2*ind+2].first){
            seg[ind] = seg[2*ind+1];
        }
        else if(seg[2*ind+1].first > seg[2*ind+2].first){
            seg[ind] = seg[2*ind+2];

        }
        else{
            seg[ind].first = seg[2*ind+1].first;
            seg[ind].second = seg[2*ind+1].second + seg[2*ind+2].second;
        }
    }
};
void solve(){
    int n,m;
    cin>>n>>m;
    vector<int> v1(n,0);
    ivec(v1);
    SegmentTree sg1(n);
    sg1.build(0,0,n-1,v1);
    for(int i=0;i<m;i++){
        int a,b,c;
        cin>>a>>b>>c;
        if(a == 1){
            sg1.update(0,0,n-1,b,c);
        }
        else{
            pair<int,int> p = sg1.query(0,0,n-1,b,c-1);
            cout<<p.first<<" "<<p.second<<"\n";
        }
    }
    
}
int32_t main(){
    ios::sync_with_stdio(false);
    cin.tie(0);
    cout.tie(0);
    solve();
    return 0;
}


