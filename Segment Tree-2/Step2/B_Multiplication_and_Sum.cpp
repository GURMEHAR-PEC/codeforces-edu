...
class LazySegTree{
    ...
    void build(int ind,int low,int high,vector<int>& arr){
        ...
        seg[ind] = (seg[(ind<<1)+1]+seg[(ind<<1)+2])%mod;

    }

    void update(int ind,int low,int high,int l,int r,int val){
        if(lazy[ind]!=1){
            seg[ind] = (seg[ind]*lazy[ind])%mod;
            if(low!=high){
                lazy[(ind<<1)+1] = (lazy[2*ind+1]*lazy[ind])%mod;
                lazy[(ind<<1)+2] = (lazy[2*ind+2]*lazy[ind])%mod;
            }
            lazy[ind]=1;
        }
        ...
        if(low >= l && high<=r){
            ...
            if(low!=high){
                lazy[(ind<<1)+1] = (lazy[(ind<<1)+1]*val)%mod;
                lazy[(ind<<1)+2] = (lazy[(ind<<1)+2]*val)%mod;
            }
            return;
        }
    ...
    int query(int ind,int low,int high,int l,int r){
        if(lazy[ind]!=1){
            seg[ind] = (seg[ind]*lazy[ind])%mod;
            if(low!=high){
                lazy[(ind<<1)+1] = (lazy[(ind<<1)+1]*lazy[ind])%mod;
                lazy[(ind<<1)+2] = (lazy[(ind<<1)+2]*lazy[ind])%mod;
            }
            lazy[ind]=1;
        }
        ...
        if(low>=l && high<=r) return seg[ind]%mod;
        int mid = (low+high)>>1;
