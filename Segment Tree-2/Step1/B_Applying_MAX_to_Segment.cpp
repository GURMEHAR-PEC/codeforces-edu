...
class LazySegTree{
    ...
    void update(int ind,int low,int high,int l,int r,int val){
        if(lazy[ind]!=0){
            ...
            if(low!=high){
                lazy[(ind<<1)+1] = max(lazy[2*ind+1],lazy[ind]);
                lazy[(ind<<1)+2] = max(lazy[2*ind+2],lazy[ind]);
            }
            lazy[ind]=0;
        }
        ...
        if(low >= l && high<=r){
            ...
            if(low!=high){
                lazy[(ind<<1)+1] = max(lazy[(ind<<1)+1],val);
                lazy[(ind<<1)+2] = max(lazy[(ind<<1)+2],val);
            }
            return;
        }
    ...
    int query(int ind,int low,int high,int l,int r){
        if(lazy[ind]!=0){
            ...
            if(low!=high){
                lazy[(ind<<1)+1] = max(lazy[ind],lazy[(ind<<1)+1]);
                lazy[(ind<<1)+2] = max(lazy[ind],lazy[(ind<<1)+2]);
            }
            lazy[ind]=0;
        }
